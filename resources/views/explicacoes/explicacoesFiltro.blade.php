        ...
        <div class="col-4 d-flex justify-content-end">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="filterDropdown">
                    <li><a class="{{ request()->get('sort') == 'price_asc' ? 'dropdown-item active' : 'dropdown-item' }}"
                           href="{{ route('explicacoes.search', ['query' => request()->query, 'sort' => 'price_asc']) }}">Preço mais baixo para mais alto</a></li>
                    <li><a class="{{ request()->get('sort') == 'price_desc' ? 'dropdown-item active' : 'dropdown-item' }}"
                           href="{{ route('explicacoes.search', ['query' => request()->query, 'sort' => 'price_desc']) }}">Preço mais alto para mais baixo</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    @if ($explicacoes->hasPages())
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                ...
                @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $explicacoes->appends(request()->query())->previousPageUrl() }}"
                           rel="prev">Anterior</a>
                    </li>
                @endif
                {{-- Pagination Elements --}}
                @foreach (range(1, $explicacoes->lastPage()) as $page)
                    ...
                    @else
                        <li class="page-item">
                            <a class="page-link" href="{{ $explicacoes->appends(request()->query())->url($page) }}">{{ $page }}</a>
                        </li>
                    @endif
                @endforeach
                {{-- Next Page Link --}}
                @if ($explicacoes->hasMorePages())
                    <li class="page-item">
                        <a class="page-link" href="{{ $explicacoes->appends(request()->query())->nextPageUrl() }}" rel="next">Próximo</a>
                    </li>
                @else
                    <li class="page-item disabled">
